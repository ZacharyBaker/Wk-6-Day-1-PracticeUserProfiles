...
module.exports = {
    ...
    addFriendProfiles: function(req, res, next){
       console.log(req.session.currentUser);
        var friendsProfilesArray = [];
        var currUser = req.session.currentUser;
        for (var i = 0; i < currUser.friends.length; i++){
           var currFriend = currUser.friends[i];
            for (var j = 0; j < profiles.length; j++){
                if (currFriend === profiles[j].name){
                    friendsProfilesArray.push(profiles[j]);
